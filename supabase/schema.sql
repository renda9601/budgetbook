...
-- 초기 데이터 순서:
-- 1. Supabase Auth에서 dad@family.local, mom@family.local 사용자를 만든다.
-- 2. 아래 seed SQL의 dad_user_id, mom_user_id 값을 실제 auth.users.id로 바꿔 실행한다.

/*
with ids as (
  select
    '00000000-0000-0000-0000-000000000001'::uuid as dad_user_id,
    '00000000-0000-0000-0000-000000000002'::uuid as mom_user_id
),
home as (
  insert into public.households (name, created_by)
  select '우리집 가계부', dad_user_id from ids
  returning id
),
profile_seed as (
  insert into public.profiles (user_id, login_id, display_name, internal_email)
  select dad_user_id, 'dad', '재용', 'dad@family.local' from ids
  union all
  select mom_user_id, 'mom', '유미', 'mom@family.local' from ids
),
member_seed as (
  insert into public.household_members (household_id, user_id, display_name, role)
  select home.id, ids.dad_user_id, '재용', 'owner' from home, ids
  union all
  select home.id, ids.mom_user_id, '유미', 'member' from home, ids
)
insert into public.cards (household_id, name, issuer, color, sort_order)
select home.id, name, name, color, sort_order
from home,
(values
  ('신한카드', '#2563eb', 1),
  ('현대카드', '#111827', 2),
  ('삼성카드', '#0ea5e9', 3)
) as card_values(name, color, sort_order);
*/
